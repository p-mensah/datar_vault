...
const TabbedInvoiceForm: React.FC<TabbedInvoiceFormProps> = ({
  invoiceData, setInvoiceData, onGenerateContract, isGenerating, error, savedClients, setSavedClients, savedItems, onGenerateTerms
}) => {
  ...
  const tabs = [
    ...
    { id: 'items', label: 'Items', icon: FileTextIcon },
    { id: 'automation', label: 'Automation', icon: RecurringIcon },
    { id: 'details', label: 'Details', icon: DollarSignIcon },
    { id: 'payment', label: 'Payment', icon: CreditCardIcon },
    { id: 'terms', label: 'Terms', icon: ClipboardDocumentCheckIcon },
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[Template.Modern, Template.Classic, Template.Creative].map(t => (
                    <button key={t} onClick={() => setInvoiceData(prev => ({ ...prev, template: t }))} className={`p-3 border rounded-lg text-sm transition-colors ${invoiceData.template === t ? 'border-slate-700 bg-slate-50' : 'border-slate-300 hover:border-slate-400'}`}>
                      {t}
                    </button>
                  ))}
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div>
              ...
              <div className="flex items-end gap-2 mb-4">
                ...
                <button onClick={handleSaveClient} title="Save current client" className="p-2 border border-slate-300 rounded-md text-slate-600 hover:bg-slate-100">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                    {CONTRACT_TYPES.map(type => (
                      <button key={type} onClick={() => handleContractDetailChange({ target: { name: 'contractType', value: type } } as any)} className={`p-3 border rounded-lg text-sm transition-colors ${invoiceData.contractDetails.contractType === type ? 'border-slate-700 bg-slate-50' : 'border-slate-300 hover:border-slate-400'}`}>
                        {type}
                      </button>
                    ))}
                  </div>
              ...
              <>
                <div className="space-y-2">
                  {invoiceData.items.map((item) => (
                    <div key={item.id} className="grid grid-cols-12 gap-2 p-3 border rounded-md bg-slate-50">
                      <div className="col-span-12 md:col-span-4">
                        <Input placeholder="Description" name="description" value={item.description} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleItemChange(item.id, e)} small />
                      </div>
                      <div className="col-span-3 md:col-span-2">
                        <Input placeholder="Qty" name="quantity" type="number" value={item.quantity} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleItemChange(item.id, e)} small />
                      </div>
                      <div className="col-span-4 md:col-span-2">
                        <Input placeholder="Unit" name="unit" value={item.unit || ''} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleItemChange(item.id, e)} small />
                      </div>
                      <div className="col-span-5 md:col-span-2">
                        <Input placeholder="Rate" name="rate" type="number" value={item.rate} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleItemChange(item.id, e)} small />
                      </div>
                      <div className="col-span-4 md:col-span-1 flex items-center justify-end text-slate-700 font-medium">
                ...
                <button onClick={addItem} className="w-full flex items-center justify-center py-2 px-4 border border-dashed border-slate-300 rounded-md text-sm font-medium text-slate-700 hover:bg-slate-50">
                  <PlusIcon className="h-5 w-5 mr-2" /> Add Blank Item
                </button>
              </>
            )}
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="border-t pt-4">
              ...
              <div className="mt-3 p-2 bg-slate-100 rounded-md text-xs text-slate-600 flex items-start">
                ...
                <span>
                  Note: For client payments, the payment reference will be automatically set to {getReferenceLabel(invoiceData.documentType)} {invoiceData.documentNumber} on the final document.
                </span>
              </div>
            </div>
          </div>
      ...
      case 'terms':
        return (
          <div className="space-y-4">
            ...
            <button onClick={handleGenerateTermsClick} disabled={isGenerating} className="w-full flex items-center justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-slate-600 hover:bg-slate-700 disabled:bg-slate-400">
...
export default TabbedInvoiceForm;